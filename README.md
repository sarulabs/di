...
Once all the definitions are added into the Builder, you can call the `Build` method to generate a `Container`.
...
The `Build` function can also use the container. That allows to build objects depending on other objects defined in the container.
...
You can add a name to the definition. It allows to retrieve the definition from its name.
...
Retrieving an object from its name instead of its definition requires a additional lookup in a `map[string]int`. That makes it significantly slower. If performance is critical for you, you should retrieve objects from their definitions.
...
You can add tags to a definition. Tags are not used internally by this library. There are only there to help you organize your definitions.
...
The third method to retrieve an object is `Fill`. It returns an error if something goes wrong like `SafeGet`, but it may be more practical in some situations. It uses reflection to fill the given object. Using reflection makes it is slower than `SafeGet`.
...
DI includes some elements to ease its integration in web applications.
